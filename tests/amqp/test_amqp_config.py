...
class Test(unittest.TestCase):
    ...
    def test_1(self):
        self.spec = AMQPConfigSpec(URLParameters("amqp://guest:guest@localhost:5672/%2f"),
                                   ...
                                   exchanges=[AMQPExchangeDescriptor(exchange=self.exchange, auto_delete=True, exchange_type="topic")],
                                   bindings=[AMQPBindDescriptor(queue=self.queue, exchange=self.exchange, routing_key='dog.*')],
